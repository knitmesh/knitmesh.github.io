...
更多关于`nc`的demo可参考我实现的[基于nc封装的端口扫描工具](https://github.com/jingh/portscanner)以及[nc命令实例](https://github.com/jingh/notes/blob/master/linux/nc%E5%91%BD%E4%BB%A4%E5%AE%9E%E4%BE%8B.md)。
...
```
fgp@controller:~$ sudo traceroute  -I -n jingh.me
traceroute to jingh.me (192.30.252.154), 30 hops max, 60 byte packets
 ...
 ...
```
可以看到，从主机到`jingh.me`共经过30跳，并统计了每一跳间的响应时间。
...
从图上可以看出从本地到`jingh.me`经过的所有路由，每一个路由间的丢包率、响应时间等。
...
具体用法参考[buptLogin](https://github.com/jingh/buptLogin)。
...
```bash
wget --mirror -p --convert-links -P jingh http://jingh.me
```

wget还支持指定下载文件的格式，比如只下载jpg图片:

```bash
wget -A.jpg -r -l 2 http://jingh.me/
```
...
```bash
fgp@controller:~/github/jingh.github.io$ sudo ipset list blacklist
Name: blacklist
Type: hash:ip
Revision: 2
